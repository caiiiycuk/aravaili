...
function showContactModal() {
    ...
    container.className = "contacts-modal";
    for (var _i = 0, buttons_1 = buttons; _i < buttons_1.length; _i++) {
        var next = buttons_1[_i];
        var el = void 0;
        if (next.label === true) {
            el = document.createElement("h2");
            el.className = "contacts-label";
            el.innerHTML = next.text;
        }
        else if (next.onClick) {
            el = document.createElement("div");
            el.className = "contacts-button button";
            el.onclick = next.onClick;
            el.innerHTML = next.text;
        }
        else {
            el = document.createElement("div");
            el.className = "contacts-exit button";
            el.innerHTML = next.text;
            el.onclick = function () {
